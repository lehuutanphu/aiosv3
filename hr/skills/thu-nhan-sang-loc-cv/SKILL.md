...
4. Server tự phát hiện tên file được nhắc tới, đính kèm thẳng nội dung PDF vào tin nhắn cho bạn đọc
   (qua OpenRouter file-parser, miễn phí) — bạn **không cần** tool đọc file, chỉ cần chấm điểm theo
   đúng rubric ở Phần B rồi lưu bằng `upsert_candidate` với `file_cv` là tên file đó.
5. Nếu CV là bản scan ảnh (không có text) và đọc không ra nội dung, báo cho user biết để họ gửi bản
   PDF có text thật hoặc paste tay nội dung.
...
1. Cập nhật `ung_vien[]` trong requisition qua tool `upsert_candidate`: `ma_uv`, `ten`, `email`,
   `sdt`, `nguon`, `ngay_nop`, `file_cv`, `diem`, `xep_loai`, `diem_manh[]`, `diem_can_hoi[]`,
   `trang_thai: "moi"`. Server **tự động ghi luôn dòng tương ứng** vào
   `hr/templates/Tuyen-dung-2026-Pipeline.xlsx` (sheet `Pipeline`, khóa `Ma_UV`+`REQ_ID`) —
